...
start = time()
train_df =  pd.read_csv(path+'train.csv', index_col = "item_id", parse_dates = ["activation_date"])
test_df =  pd.read_csv(path+'test.csv', index_col = "item_id", parse_dates = ["activation_date"])
print(train_df.shape, test_df.shape)
print('Time taken for the loading dataset is:', time()-start )
...
print ('Handling the missing value in datasets.....')
...
print('Time taken for handling missing value is:', time()-start )



# Normalize price and ScaAvito_MLP_v14.pyle target variable to log.

train_df['price'] = np.where((train_df['price'] <= 3),3,train_df.price)
...
MAX_TEXT = np.max([np.max(full_df.seq_name.max()), np.max(full_df.seq_item_description.max()),]) + 4
...
def get_keras_data(df):
    ...
    X = {'name': pad_sequences(df.seq_name, maxlen=MAX_NAME_SEQ),
...
def new_rnn_model(lr=0.001, decay=0.0):
    ...
    main_l = Dense(128)(ma# Importing dependencies
...
# Normalize price and Scale target variable to log.
...
print ('Creating the data for the Ridge Model')
...
print("Vectorizing data for Ridge...")
...
vectorizer = FeatureUnion([
    ...
    ('name', CountVectorizer(ngram_range=(1, 2),max_features=50000,preprocessor=build_preprocessor('name'))),
...
print('Time taken for preparing the data for Ridge:', time()-start )
...
ridge_model = Ridge(solver='sag', fit_intercept=True, alpha=0.5,max_iter=200, normalize=False, tol=0.04,)

ridge_model.fit(X_train, Y_train)

print('Time taken for fitting the Ridge model :', time()-start )
...
submission = pd.DataFrame({"test_id": test_df.test_id,"price": preds.reshape(-1),})

submission.to_csv("Vishyrnnridge_subs.csv", index=False)

print ('Submissions are ready')in_l)
